...
//!
//! Linux inotify (unlike kqueue) does fire `IN_MODIFY` on the parent directory's inode when its
//! child file's content changes via an in-place rewrite *if* the directory is registered for
//! `IN_MODIFY`. But the engine does NOT register `IN_MODIFY` on a Dir under the translator (CONTENT
//! × Dir is a no-op; STRUCTURE on Dir adds `IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO`,
//! not `IN_MODIFY`). So a STRUCTURE-only Dir watch on Linux behaves similarly to kqueue: an
//! in-place edit at a child file fires no parent-dir event. The fix is identical — install a
//! per-file watch with CONTENT.
//!
//! This test pins the **kernel + watcher + translator** half of the closure: with both a STRUCTURE
//! Dir watch and a CONTENT per-file watch installed, an in-place truncate-and-rewrite fires
//! `ContentChanged` on the file's wd and (no event) on the dir's wd. The engine half is covered by
//! `crates/specter-engine/tests/event_filtering.rs`.
...
/// Drain whatever the watcher emits for `dur` into `out`, without asserting on a specific event.
/// Used by tests that want to collect the raw `WatcherEvent` stream (incl. overflow) over a window.
...
/// (`STRUCTURE | CONTENT`):
/// - Parent dir watched with STRUCTURE (Dir-only mask: `IN_CREATE | IN_DELETE | IN_MOVED_FROM |
///   IN_MOVED_TO`).
/// - Per-leaf file watched with CONTENT (File-only mask: `IN_MODIFY | IN_CLOSE_WRITE`).
#[test]
fn in_place_edit_fires_content_changed_on_per_file_wd() {
    ...
    // In-place edit. `std::fs::write(path, ...)` opens with `O_TRUNC` and writes — same syscall
    // pattern as `echo 'test' > file.txt`. No rename, no unlink. The kernel emits `IN_MODIFY` and
    // `IN_CLOSE_WRITE` on the file's inode; the watcher's per-batch dedup collapses both to one
    // `ContentChanged`.
    ...
    // Bonus: per-batch dedup should collapse the kernel's `IN_MODIFY + IN_CLOSE_WRITE` pair to one
    // `ContentChanged` per write — count and assert.
...
/// Confirm the symptom: a STRUCTURE-only Dir watch (no per-file wd) does NOT fire on in-place file
/// edits. The Dir's STRUCTURE mask covers only child create / delete / move; an in-place edit at a
/// child file is observable via the file's `IN_MODIFY` registration, not the dir's.
#[test]
fn in_place_edit_does_not_fire_on_structure_only_dir_watch() {
    ...
    // The dir's STRUCTURE-only watch must not fire on a child's in-place edit. STRUCTURE on Dir
    // installs only `IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO`; child content modify
    // happens via `IN_MODIFY` on the child's inode, not the parent's.
