//! Non-blocking-drain semantics on the inotify watcher.
//!
//! The trait contract: [`FsWatcher::drain_ready`] is non-blocking, [`AsFd::as_fd`] is the readiness
//! substrate, and the caller blocks via a reactor (mio::Poll) on the fd. Linux twin of
//! `kqueue_wake.rs` — pins the same contract against the kernel-backed inotify fd.
...
/// poll → drain → idle" sequence the production driver exercises against a real inotify fd. Linux
/// twin of `kqueue_wake::poll_then_drain_returns_kernel_events`.
#[test]
