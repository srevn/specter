...
//! destination file's inode atomically. The watched file's old inode is
//! unlinked → terminal [`FsEvent`] (`Removed` or `Renamed`); the watched
//! parent dir sees a [`FsEvent::StructureChanged`] (the kernel emits
//! `IN_MOVED_TO` on the rename's destination component, which the
//! translator routes through `IN_CREATE | IN_DELETE | IN_MOVED_FROM |
//! IN_MOVED_TO` → `StructureChanged`).
//!
//! The engine vacates the slot on the terminal event; the next dir
//! `StructureChanged` triggers a reprobe that discovers the new file at
//! the same path with a fresh `ResourceId`. Mirror of
...
fn atomic_save_emits_terminal_on_old_inode_and_structure_on_dir() {
    ...
    // Dir watch needs STRUCTURE so the kernel registers
    // `IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO` on the
    // directory and a child rename produces `StructureChanged`. The file
    // watch can stay with EMPTY events: the identity floor
    // (`IN_DELETE_SELF | IN_MOVE_SELF | IN_UNMOUNT`) covers the terminal
    // event we assert on for the file's old inode.
    w.watch(r_dir, tmp.path(), ResourceKind::Dir, ClassSet::STRUCTURE)
        .expect("watch dir");
    w.watch(r_file, &target, ResourceKind::File, ClassSet::EMPTY)
        .expect("watch file");

    // Register the watcher's inotify fd with mio; `drain_ready` is
    // non-blocking by trait, so the caller blocks via the reactor.
