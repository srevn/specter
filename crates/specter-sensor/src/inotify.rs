...
//!
//! The two watchers share the FFI surface but own *separate* inotify
//! fds — independent kernel queues, no cross-talk. They are
//! deliberately decoupled so the engine watcher's per-resource
//! bookkeeping (slotmap, wd-routing table) stays out of the config
//! watcher's drain path and vice versa. Each inotify fd is exposed
