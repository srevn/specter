...
//!
//! Per `inotify(7)`, the kernel emits a stream of variable-length records on `read(inotify_fd)`:
//!
//! ```text
...
//! ```
//!
//! Each field is in native byte order (the kernel's ABI is host-native; see `linux/inotify.h`). The
//! parser reads through `from_ne_bytes`, which works on any Linux architecture without endianness
//! ceremony.
//!
//! ## What v1 keeps and what it throws away
//!
//! v1 collapses every name-bearing structure event into [`specter_core::FsEvent::StructureChanged`]
//! and re-discovers the delta via a parent probe — `cookie` and `name` are never consumed. The
//! parser preserves them anyway so a future optimization that mints a typed `Input::ChildEvent {
//! parent, name, kind }` directly from inotify does not have to reshape the record API. Cost is one
//! byte-slice borrow per record on the hot path.
//!
//! ## Truncation discipline
//!
//! The kernel returns `EINVAL` if the user buffer is below the per-event minimum (`sizeof(struct
//! inotify_event) + NAME_MAX + 1` ≈ 273 bytes, per `inotify(7)`). The watcher sizes its drain
//! buffer well above that — see [`super::watcher`] — so `read` never returns a truncated record.
//! The defensive guards below silently end iteration on a malformed buffer rather than panic, but
//! those branches are observationally dead under healthy invariants.

use libc::c_int;

/// Size of the fixed-width header preceding the variable-length name. Bound to
/// [`libc::inotify_event`] so a future libc binding change (the kernel ABI itself is stable) becomes
/// a compile-time concern rather than a silent drift between our parser and the kernel layout.
const HEADER_SIZE: usize = std::mem::size_of::<libc::inotify_event>();

/// One inotify record borrowed from the watcher's drain buffer.
///
/// `name` is the basename portion with trailing NUL padding stripped: non-name records
/// (`IN_DELETE_SELF`, `IN_UNMOUNT`, etc.) report `len = 0`, surfaced here as an empty slice.
#[derive(Debug, Clone, Copy)]
pub(super) struct Record<'a> {
    /// Watch descriptor identifying which install fired this event. Maps to `ResourceId` via the
    /// watcher's `wd → r` index.
    pub wd: c_int,
    /// Bit set of `IN_*` flags; consumed by [`super::normalize`].
    pub mask: u32,
    /// Pairing token for `IN_MOVED_FROM` ↔ `IN_MOVED_TO`. v1 does not match move pairs (the engine
    /// probes the parent on `StructureChanged`); preserved for v3+ named-event handling.
    #[allow(dead_code)]
    pub cookie: u32,
    /// Basename portion (NUL padding stripped). Empty for non-name events. Borrows from the
    /// watcher's drain buffer; lifetime matches the parser's input slice.
    pub name: &'a [u8],
}

/// Iterate every record packed into `buf`. Iteration ends at the first truncated record; under
/// healthy invariants `buf` always ends on a record boundary, so the iterator drains exactly the
/// populated prefix.
#[must_use]
...
impl<'a> Iterator for RecordIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        // Defensive: a truncated header is impossible from a real `read(inotify_fd)` (the kernel
        // never half-fills a record), but ending iteration silently is safer than panicking on a
        // fuzzed input.
        ...
        // `len` is `__u32` per `linux/inotify.h`; widening to `usize` is exact on 64-bit
        // (specter-sensor's only Linux target — see the 32-bit compile_error in
        // `super::super::inotify`).
        ...
        let payload_end = match HEADER_SIZE.checked_add(len) {
            ...
            // Either the kernel claimed a length the buffer can't honour (truncation — see the
            // discipline note in the module header), or `len` arithmetic overflowed `usize`
            // (impossible on 64-bit but defended for completeness).
            _ => return None,
        };

        let name_with_padding = &self.remaining[HEADER_SIZE..payload_end];
        // Strip trailing NUL padding to get the basename. The kernel pads to align the next record;
        // `position` returns `None` only when `len = 0` (no name), in which case `name` is an empty
        // slice.
...
mod tests {
    ...
    /// Convenience: encode one record into a `Vec<u8>` ready for [`parse`]. Mirrors what the kernel
    /// writes onto the inotify_fd's read buffer (native byte order, padded name).
    ...
    fn cookie_paired_records_round_trip() {
        // IN_MOVED_FROM and IN_MOVED_TO share a cookie under the kernel's pairing protocol; v1
        // doesn't pair them but the parser preserves the cookie for v3+ named-event handling.
    ...
    fn iter_advances_through_padded_records_in_order() {
        // Three back-to-back records with varying name lengths and padding; verify the iterator
