//! inotify mask + kind → [`FsEvent`]. Mirror of [`crate::kqueue::normalize`].
//!
//! A single inotify event may carry multiple bits (the kernel coalesces `IN_MODIFY | IN_CLOSE_WRITE`
//! on close, for instance). We emit at most one [`FsEvent`] per record by priority order:
//!
//! ```text
//! Revoked > Removed > Renamed > StructureChanged > ContentChanged > MetadataChanged
//! ```
//!
//! The terminal flags (`IN_UNMOUNT` / `IN_DELETE_SELF` / `IN_MOVE_SELF`) are slot-final — once one
//! fires, no further events arrive on the watch descriptor. Pairing a non-terminal flag with a
//! terminal one reports the terminal: emitting both would be noise the engine doesn't act on. The
//! non-terminal `ContentChanged` / `MetadataChanged` coalescing is acceptable because the engine's
//! `Settling` state debounces either as "something changed; reschedule."
//!
//! ## `IN_IGNORED`
//!
//! `IN_IGNORED` is sensor-internal — the watcher's [`super::watcher::InotifyWatcher`]'s `drain_ready`
//! consumes it before invoking this translator. The defensive guard returning `None` is kept so a
//! slipped record (impossible under healthy invariants) is silently dropped rather than mis-routed.
//!
//! ## kqueue parity
//!
//! kqueue's `NOTE_LINK` is kind-aware (Dir → StructureChanged, File → MetadataChanged). inotify has
//! no `NOTE_LINK` analogue; hardlink count changes surface as `IN_ATTRIB` regardless of kind, so
//! the kind-disambiguating branch the kqueue normalizer carries is collapsed: `IN_ATTRIB` always
//! yields `MetadataChanged`. The `Dir`-vs-`File` branch in the `IN_MODIFY | IN_CLOSE_WRITE` arm is
//! defensive — the translator never registers those bits on a Dir, but a kernel anomaly that
//! delivered them would still land on a sensible `StructureChanged`.
...
/// Map a single inotify record's mask + the watcher's cached kind to at most one [`FsEvent`].
/// Returns `None` when:
///
/// - `IN_IGNORED` is set (sensor-internal cleanup signal),
/// - the mask carries no actionable bit (registration ack on a record the kernel emitted with only
///   orientation flags like `IN_ISDIR`).
///
/// Pure / `const fn`: branches only on its inputs.
#[must_use]
pub(super) const fn mask_to_fs_event(mask: u32, kind: ResourceKind) -> Option<FsEvent> {
    // IN_IGNORED is sensor-internal — the watcher consumes it before calling here. Defensive guard:
    // a slipped IN_IGNORED returns None so it can't masquerade as a class-bearing event.
    ...
    // Name-bearing structure events: parent's child set changed. v1 collapses the cookie + name;
    // the engine probes the parent on every StructureChanged to discover what changed by name.
    if mask & (IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO) != 0 {
        return Some(FsEvent::StructureChanged);
    }

    // Resolve `Unknown` once via the canonical collapse — single source of truth shared with the
    // translator and the engine's class filter.
    let effective = kind.effective();

    // Non-terminal CONTENT bits collapse on kind. The translator requests these only on File
    // watches, so the Dir arm is defensive (kernel-anomaly safety net): a stray IN_MODIFY on a Dir
    // folds into StructureChanged, matching kqueue's NOTE_WRITE-on-Dir policy.
...
mod tests {
    ...
    fn modify_close_write_on_dir_yields_structure_changed_defensively() {
        // The translator never registers MODIFY/CLOSE_WRITE on Dir, but a kernel anomaly that
        // delivered one should still land on a sensible signal. Mirrors kqueue's NOTE_WRITE-on-Dir
        // rule.
        for m in [IN_MODIFY, IN_CLOSE_WRITE] {
            assert_eq!(
                mask_to_fs_event(m, ResourceKind::Dir),
    ...
    fn ignored_returns_none() {
        // IN_IGNORED dominates even paired with a class-bearing bit; the watcher consumes it
        // pre-normalize, but the defensive guard catches any slipped record.
