...
//!   the parent path is gone; auto-reload cannot recover.
//!   `drain_ready()` propagates `Err`; the bin's wrapper logs and
//!   exits the watcher thread; SIGHUP-only operation continues.
//!   Parent-dir loss / parent-symlink retarget are documented
//!   restart-required limitations.
//!
...
//!
//! After the drain-to-empty loop completes, [`Self::drain_ready`]
//! runs [`Self::try_reopen`] iff a basename-matched parent record
//! was observed *and* the final `file_wd` state is `None`. The
//! decision is post-loop-of-loops, so the intra-batch ordering of
//! the parent's `IN_MOVED_TO` / `IN_CREATE` and the file's
//! `IN_IGNORED` (the kernel reap of the old wd) is irrelevant, and
//! so is the kernel splitting one logical atomic-save burst across
//! two `read_inotify` calls within a single `drain_ready`
//! invocation — every ordering converges on the same post-loop
//! state.
//!
//! Post-loop is also a borrow-checker concession: [`record::parse`]
//! borrows [`Self::read_buf`] for the iterator's lifetime; a
//! `&mut self` method invocation inside the loop would conflict
//! with that borrow. Disjoint-field NLL handles the field-level
//! mutation on `file_wd` (Copy `Option<i32>`) inline; the post-loop
//! `try_reopen` call runs after the borrow ends.
//!
//! `saw_basename_parent` (not bare `real_seen`) gates the recovery
//! because file-side records nulling `file_wd` mid-drain are not a
...
//! - `inotify_fd` ([`OwnedFd`]) drops first → the kernel reaps both
//!   per-watch descriptors on this instance (per `inotify(7)`) and
//!   queues the resulting `IN_IGNORED` records on a stream no
//!   consumer reads (benign).
//! - `file_wd`, `parent_wd`, `parent_path`, `config_basename`,
//!   `read_buf` — `Option<c_int>` / `c_int` integers and heap
//!   strings / buffer; no kernel obligations of their own. Closing
//!   the inotify fd above is the sole deregister signal.
...
/// observes them in `drain_ready()` regardless. Setting them here
...
/// `drain_ready()` drain handles it explicitly.
///
...
/// see the module docs for the full lifecycle and edge-case matrix.
/// Construct with [`Self::new`]; drive with
...
pub struct InotifyConfigWatcher {
    ...
    /// (per `inotify(7)`). Exposed through [`AsFd`] so a reactor can
    ...
    /// the full file path, and by `drain_ready()` for the
    /// parent-event basename filter
    /// (`rec.name == self.config_basename.as_bytes()`). Stored as
    /// `OsString` to round-trip raw bytes losslessly across non-UTF-8
    /// names.
    config_basename: OsString,

    /// Drain buffer for `read_inotify`. Sized at construction and
    /// reused across `drain_ready` calls — the hot path performs no
    /// allocation.
    read_buf: Vec<u8>,
}
...
impl ConfigWatcher for InotifyConfigWatcher {
    /// Non-blocking drain-to-empty of the inotify queue. Loops on
    /// [`ffi::read_inotify`] (which under `IN_NONBLOCK` returns
    /// `Ok(0)` on an empty queue) until the kernel signals
    /// `EAGAIN`, dispatching each record:
    ///
    ...
    ///
    /// After the drain-to-empty loop completes, `try_reopen` runs
    /// iff a basename-matched parent record was observed *and* the
    /// final `file_wd` state is `None`. The recovery decision lives
    /// outside the loop on purpose: the kernel may split one logical
    /// atomic-save burst across two `read_inotify` batches
    /// (basename-matched parent record in batch *k*, file-wd
    /// `IN_IGNORED` in batch *k+1*), and an inside-the-loop
    /// `try_reopen` would race the in-progress inode swap. The
    /// post-loop placement makes the decision against the
    /// invocation's final state, independent of both the intra-batch
    /// record order and the kernel's across-batch fragmentation.
    /// See the module's "Opportunistic re-open" section.
    ///
    /// The parent-`IN_IGNORED` arm intentionally returns mid-loop:
    /// parent-dir loss is terminal, and continuing the drain on a
    /// torn-down watch would only consume `IN_IGNORED`-tail stragglers
    /// against an already-doomed state.
    ///
    /// `EINTR` is retried inside [`ffi::read_inotify`]. Any other
    ...
    fn drain_ready(&mut self) -> io::Result<bool> {
        ...
        // The inner iterator borrows `self.read_buf` for its scope.
        ...
        // post-loop, after every borrow ends.
        //
        // Per-record dispatch parallels the engine watcher
        // (`super::watcher::InotifyWatcher::drain_ready`):
        // control-records first (`IN_Q_OVERFLOW`, `IN_IGNORED`),
        // then per-wd routing.
        loop {
            let n_bytes = ffi::read_inotify(&self.inotify_fd, &mut self.read_buf)?;
            if n_bytes == 0 {
                // EAGAIN under IN_NONBLOCK ⇒ kernel queue drained;
                // edge-triggered contract satisfied.
                break;
            }
            for rec in record::parse(&self.read_buf[..n_bytes]) {
                // 1. IN_Q_OVERFLOW: kernel-emitted overflow signal. The
                //    config watcher's per-instance queue is normally
                //    not at risk (a handful of events per editor save),
                //    but paranoia is cheap. Surface as a generic pulse
                //    so the driver's lstat filter takes the hit on
                //    uncertain state. wd is `-1` per `inotify(7)`,
                //    falling outside `file_wd` / `parent_wd` — ordering
                //    relative to `IN_IGNORED` is intent-preserving, not
                //    load-bearing.
                ...
                if rec.mask & libc::IN_IGNORED != 0 {
                    if Some(rec.wd) == self.file_wd {
                        ...
                        // Terminal: parent watch is gone, no recovery.
                        // Drop the in-flight partial state (real_seen,
                        // saw_basename_parent) and propagate Err so
                        // the bin tears the watcher thread down.
                        return Err(io::Error::other("config parent dir watch lost"));
                    }
                    // Stale wd (already-nullified file_wd or
                    // unknown): drop silently. No multi-resource
                    // routing here.
                    continue;
                }
        ...
        // state of `file_wd` after the entire drain-to-empty loop.
        // A basename-matched parent record is the proof "the
        // watched basename is back"; a bare file-side `IN_IGNORED`
        // without a parent event is not a recovery signal (the file
        // is gone, recreate hasn't happened yet), so we gate on
        // `saw_basename_parent` rather than the weaker `real_seen`.
        // `try_reopen` is idempotent and `ENOENT`-fast; a call on a
        // not-yet-recreated file is cheap and the next
        // basename-matched parent event retries.
...
mod tests {
    ...
    /// for an inotify drain on any sane CI host while still bounding
    /// a stuck test below CI's per-test timeout.
    fn watchdog() -> Instant {
    ...
    fn wakes_on_atomic_save() {
        ...
        // coalesced drain containing the parent's basename-matched
        // `IN_MOVED_TO` plus the file wd's `IN_IGNORED` restores
        // `file_wd` in this same drain regardless of intra-batch
    ...
    fn wakes_on_delete_recreate() {
        ...
        // satisfy Ok(true). Multiple drain calls may be needed if
        ...
        // basename match → try_reopen runs after the drain loop.
    ...
    fn basename_filter_rejects_sibling_writes() {
        ...
        // The fd becomes readable, but the basename filter inside
        // drain_ready drops the record; the drain returns
        // `Ok(false)`. file_wd's inode is unaffected.
    ...
    fn parent_wd_lost_returns_err() {
        ...
        // on the parent's wd; `drain_ready` propagates `Err` on the
        // IN_IGNORED.
