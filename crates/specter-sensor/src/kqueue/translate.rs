...
//!
//! Single platform-specific translation point. Backend-agnostic [`ClassSet`] enters;
//! kqueue-specific `u32` fflags leave. The identity floor is OR-ed unconditionally onto every
//! registration so the engine's reconciler always sees terminal events (delete / rename / revoke),
//! independent of the user's class mask.
//!
//! Inotify's analogue would live in a sibling module `sensor::inotify::translate`; the engine
...
//!
//! `NOTE_LINK` placement is kind-aware: on a Dir it's a structural signal (subdirectory `..`
//! backref count change); on a File it's a metadata signal (hardlink count change).
//!
//! `ResourceKind::Unknown` (defensive path; the sensor's own `fstat` couldn't classify the inode as
//! Dir or Reg) is treated as File via [`ResourceKind::effective`] — single source of truth for that
//! convention. The engine's reconciler stamps the correct kind on its next probe; if that flips
//! Dir↔File the engine emits Unwatch + Watch and the FD is reopened with the corrected mask. This
//! branch is effectively dead code in v1's flow but kept as forward-compatible defense.
...
/// Identity floor — OR-ed onto every kqueue vnode registration regardless of the user's `events`
/// mask. These three NOTE bits drive **Tree integrity** (slot vacate on delete) and **watch
/// lifecycle** (re-register on rename) and must always reach the engine even when the user opted
/// out of CONTENT / STRUCTURE / METADATA. The engine's entry filter then folds terminal events into
/// the appropriate class for per-Profile filtering (CONTENT for files, STRUCTURE for dirs); see
/// `engine::transitions::fs_event_to_class`.
pub(super) const IDENTITY_FLOOR: u32 = NOTE_DELETE | NOTE_RENAME | NOTE_REVOKE;

/// Compute the kqueue fflags mask for a vnode registration given the user's [`ClassSet`] and the
/// resource's kind. The result always contains [`IDENTITY_FLOOR`].
///
/// Pure function: no I/O, no allocation, branches only on its inputs. `const` so the call in
/// `KqueueWatcher::watch` can be inlined and constant-folded for fixture cases.
#[must_use]
pub(super) const fn class_set_to_fflags(events: ClassSet, kind: ResourceKind) -> u32 {
    let mut fflags = IDENTITY_FLOOR;
    // Resolve the Unknown→File collapse once at the entry point; the branches below decide on
    // `effective` so the convention isn't re-encoded per arm.
    ...
    // METADATA — both kinds. NOTE_LINK lands here on Files; Dir's NOTE_LINK is in the STRUCTURE
    // branch above and we don't re-add it here even when STRUCTURE isn't set, because LINK on a Dir
    // is not a metadata signal.
...
mod tests {
    ...
    /// that drops a bit is a test failure.
    #[test]
    fn identity_floor_includes_delete_rename_revoke() {
        ...
        // Floor does NOT include WRITE/EXTEND/ATTRIB/LINK — those are class-gated.
    ...
    /// EMPTY × any-kind always degrades to identity-floor only. This is the v1 fallback for
    ...
    fn metadata_on_dir_adds_attrib_only() {
        // NOTE_LINK on a Dir is STRUCTURE, not METADATA. So a METADATA-only Dir does NOT register
        // LINK.
    ...
    fn structure_and_content_on_file_only_content_applies() {
        // STRUCTURE | CONTENT on File: CONTENT wins; STRUCTURE is no-op. This is the
        // `DEFAULT_SUBTREE_ROOT` × file-leaf case.
    ...
    fn all_classes_on_file_yields_full_file_mask() {
        ...
        // File gets CONTENT (W|E) + METADATA (A|L); STRUCTURE is no-op. NOTE_LINK lands once via
    ...
    /// Re-translation determinism: the function is pure, so the same inputs always yield the same
    /// fflags. This protects the diff-skip optimization in `KqueueWatcher::watch`: two equal
    /// `(events, kind)` pairs must produce identical u32s.
    #[test]
