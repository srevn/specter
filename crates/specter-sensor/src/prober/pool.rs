...
///
/// The subtree walker recurses one Rust frame-triple per directory level
/// (`snapshot_dir → enumerate_dir → build_dir_child → snapshot_dir`),
/// bounded only by the filesystem's `PATH_MAX`: the walker addresses
