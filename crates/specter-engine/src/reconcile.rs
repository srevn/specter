...
use specter_core::{
    ChildEntry, DirChild, DirSnapshot, EntryKind, Profile, ResourceId, ResourceKind, ResourceRole,
    StepOutput, Tree, TreeSnapshot, splice,
};
use std::sync::Arc;
...
/// `Profile.has_per_file_sub == true`** (B2).
///
/// Two-phase order:
...
pub(crate) fn walk_pair(
    ...
    if let Some(p) = prior {
        for (name, prior_child) in p.entries.iter().rev() {
            ...
            if delete {
                delete_child(tree, profile, current_id, name.as_str(), prior_child, out);
            }
        }
    }
    ...
    for (name, new_child) in &new.entries {
        ...
        if !identity_match {
            ...
            create_child(tree, profile, current_id, name.as_str(), new_child, out);
            continue;
        }
        ...
        if let (Some(ChildEntry::Dir(p_dc)), ChildEntry::Dir(n_dc)) = (prior_child, new_child) {
            ...
            {
                walk_pair(Some(ps), ns, child_id, profile, tree, out);
            }
...
pub(crate) fn graft(
    ...
    // or a path that became covered between probes.
    let prior = profile
        .current
        .as_ref()
    ...
    // Emit Watch ops + materialise / reap Tree slots.
    walk_pair(prior.as_deref(), &response_arc, target, profile, tree, out);
    ...
    // short-circuits when per-level hashes match.
    let new_current = splice(profile.current.take(), target, response_arc, tree);
    profile.current = Some(new_current);
}
...
/// released on the 0→0 edge for covered Dirs (always) and covered Leaves
/// under `profile.has_per_file_sub` (B2). Idempotent on missing slots.
///
...
fn delete_child(
        ...
        && let Some(sub) = dc.subtree.as_deref()
    {
        for (cname, cchild) in sub.entries.iter().rev() {
            delete_child(tree, profile, resource, cname.as_str(), cchild, out);
        }
    }
    ...
    let releases_watch = match prior_child.kind() {
        ...
        EntryKind::File | EntryKind::Symlink | EntryKind::Other => {
            covers(profile, resource, tree) && profile.has_per_file_sub
        }
    };
    if releases_watch {
        sub_watch_demand(tree, resource, out);
    }
...
/// observed. Emits `add_watch_demand` on the 0→1 edge for covered Dirs
/// (always) and covered Leaves under `profile.has_per_file_sub` (B2).
fn create_child(
    tree: &mut Tree,
    profile: &Profile,
    parent: ResourceId,
    ...
    if covers(profile, resource, tree) {
        let need_watch = matches!(new_child, ChildEntry::Dir(_)) || profile.has_per_file_sub;
        if need_watch {
            add_watch_demand(tree, resource, out);
        }
    }
    ...
    if let ChildEntry::Dir(dc) = new_child
        && let Some(sub) = dc.subtree.as_deref()
    {
        walk_pair(None, sub, resource, profile, tree, out);
    }
}
...
mod tests {
    ...
    use specter_core::{
        ChildEntry, DirChild, DirMeta, DirSnapshot, EntryKind, LeafEntry, Profile, ProfileMap,
        ResourceId, ResourceKind, ResourceRole, ScanConfig, StepOutput, Tree, TreeSnapshot,
        WatchOp,
    };
    ...
    fn anchor(per_file: bool) -> (Tree, ProfileMap, ResourceId, specter_core::ProfileId) {
        ...
        if per_file {
            profiles.get_mut(pid).unwrap().has_per_file_sub = true;
        }
        (tree, profiles, r, pid)
    }
    ...
    fn walk_pair_prior_none_creates_dir_and_leaf_entries() {
        ...
        // Profile (has_per_file_sub=false) ⇒ Dir gets Watch, Leaf does not.
    ...
    fn walk_pair_per_file_profile_emits_watch_for_leaf_create() {
        // has_per_file_sub=true ⇒ Leaf creates *also* get Watch (B2).
    ...
    fn walk_pair_per_file_leaf_deletion_releases_watch() {
        // PerStableFile Profile: covered Leaf carries a Watch contribution;
        // delete should release it (B2 symmetric to create).
        let (mut tree, profiles, root, pid) = anchor(true);
        let leaf_id = tree.ensure(Some(root), "a.rs", ResourceRole::User);
        tree.get_mut(leaf_id).unwrap().kind = ResourceKind::File;
        crate::refcounts::add_watch_demand(&mut tree, leaf_id, &mut StepOutput::default());
    ...
    fn walk_pair_inode_change_deletes_then_creates() {
        ...
        let (mut tree, profiles, root, pid) = anchor(false);
        // Pre-materialise the prior Dir slot.
        let foo_id = tree.ensure(Some(root), "foo", ResourceRole::User);
        tree.get_mut(foo_id).unwrap().kind = ResourceKind::Dir;
        crate::refcounts::add_watch_demand(&mut tree, foo_id, &mut StepOutput::default());
