//! Oracle (a): the driver's same-tick recency coalescing is *outcome-equivalent* to processing
//! every queued event.
//!
//! `EngineDriver::drain_sensor` collapses a within-tick run of a same-`(ResourceId, FsEvent)`
//! recency hint to its first occurrence, flushing the dedup horizon at every barrier (identity
//! FsEvent or any non-FsEvent). This test mirrors that exact rule, then feeds the FULL list and the
//! RULE-COALESCED list into two engines built identically, drives both through one real Standard
//! burst lifecycle (settle → pre-fire verify), and asserts they reach the same observable outcome:
//! same probe target (the LCA of the accumulated dirty set — the quantity that would diverge if a
//! distinct `(resource, event)` were wrongly dropped, an F1-key regression, or a barrier failed to
//! flush), same terminal `ProfileState`, same settled baseline.
//!
...
/// One element of a within-tick input sequence. The recency variants are exactly the class
/// `drain_sensor` coalesces; `Barrier` is its horizon-flush case (an identity FsEvent —
/// `is_recency()` is false).
...
enum Ev {
    ...
    StructRoot,
    /// Identity FsEvent at an unwatched resource: a true barrier (non-recency) that the engine
    /// resolves to a pure no-op, so the only thing it changes is the dedup horizon.
    Barrier,
}
...
/// The production collapse rule, mirrored from `EngineDriver::drain_sensor`: keep the first
/// occurrence of each `(resource, event)` recency hint; drop later same-key duplicates; every
/// barrier clears the horizon and is itself always kept.
...
/// A fresh engine with a recursive subtree-root Sub on Dir `src`, seeded to a pinned `Idle`
/// baseline of `{a, b}` covered Files. Returns the engine, profile, the three live resources, the
...
/// Step every input at one shared `now` (one tick), force the Batching settle, capture the emitted
/// probe's target, then run the pre-fire verify to a terminal `Idle`. Returns the observable
/// outcome: `(probe target, terminal state discriminant, settled baseline hash)`.
fn drive(inputs: &[Input]) -> (Option<PathBuf>, ProfileStateDiscriminant, Option<u128>) {
    ...
    // Force the settle, keeping the StepOutput that carries the probe (drain_due discards outputs).
    // All recency inputs drive Batching; its settle expiry emits the Verifying probe whose target
    // is the LCA of the accumulated dirty set.
    let at = now + SETTLE * 4;
    ...
    // The probe is in flight (≥1 recency input ⇒ a burst started). Settle-spaced hash-equal samples
    // ⇒ Stable; the Sub already fired during seed ⇒ B1 dedup ⇒ finish to Idle, baseline intact.
...
proptest! {
    ...
    /// Coalesced ≡ full on every observable the lossy-hint contract must preserve, over random
    /// within-tick interleavings.
    #[test]
    fn coalesced_drain_matches_full_drain(seq in arb_seq()) {
        // Resolve against a throwaway fixture purely to map `Ev`s to `Input`s with stable resource
        // ids (every `seeded()` builds the same tree, so the ids match the ones `drive` resolves).
        let (_e, _pid, root, a, b, _snap, _t) = seeded();
        let full: Vec<Input> = seq.iter().map(|ev| input_of(*ev, root, a, b)).collect();

        // At least one recency hint ⇒ a burst actually starts (else both lists are barriers-only:
        // trivially equivalent, but no probe to compare — out of this property's scope).
