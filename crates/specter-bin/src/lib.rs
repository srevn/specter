...
///
/// Routes every parsed top-level subcommand to its implementation:
/// `Run` drives the daemon; the remaining variants are operator-
/// client one-shots that connect to the running daemon over the
/// IPC socket.
///
/// `Cli` is taken by value because `app::run` consumes `DaemonArgs`
/// (config moves into the driver; concurrency knobs are extracted
/// then dropped). The client arms borrow their args.
///
/// The match is exhaustive without a catch-all — adding a new
/// [`Command`] variant is a compile error here, surfacing the
/// missing handler at compile time rather than at runtime.
#[must_use]
