...
impl Loader {
    ...
    /// doesn't loop against an already-applied edit. The `meta` value
    /// is captured atomically with the parsed bytes
    /// ([`Config::from_path_with_meta`]), so a concurrent `rename(2)`
    /// can't rotate it out from under the content this snapshot
    /// represents.
    ...
    /// Meta-only rotation on parse-failure with a successful post-fail
    /// lstat. Closes the chmod-EACCES recovery loop: without this
    /// rotation, stored meta would freeze at the pre-tighten state,
    /// and the post-loosen lstat would compare equal — silently
