...
//!   through one method ([`WakingSink::send`]). The sink's [`Drop`]
//!   is the structural dual on the *channel close* edge: it closes
//!   the `Sender<Input>` BEFORE pulsing the wake, so the driver's
//!   next `try_recv` observes `Disconnected` and the actuator-gone
//!   path in [`crate::driver::Reactor::poll_and_drain`] routes
//!   through to [`crate::driver::EngineDriver::begin_shutdown`]
//!   without a race window where the driver wakes on an Empty channel
//!   and re-sleeps before the disconnect lands.
//!
...
///
/// **Drop runs close-THEN-wake**, the symmetric protocol on the
/// channel-close edge. The bin's actuator-thread closure owns a
/// `Box<dyn EffectCompleteSender>` whose inner sink is this struct;
/// when that closure exits (clean or panic), the Box drops, the
/// adapter drops, and this [`Drop`] fires. The [`Sender<Input>`]
/// closes FIRST (via [`Option::take`] + implicit end-of-statement
/// drop), so by the time the wake edge fires the driver's next
/// `try_recv` on the paired `Receiver` is guaranteed to see
/// `TryRecvError::Disconnected` — the actuator-gone signal in
/// [`crate::driver::Reactor::poll_and_drain`] (via
/// [`crate::driver::reactor::DrainedTick::actuator_gone`]) lands on
/// the next tick body. Reversing the order opens a race window
/// where the driver wakes, observes `Empty`, sleeps again, and
/// strands on Disconnected until an unrelated wake fires.
///
/// The `Option<Sender<Input>>` wrapper is layout-free for the
/// channel-sender shape ([`Sender<Input>`] is a thin
/// `Arc<Channel<Input>>` internally, so `Option<Sender<Input>>`
/// gets the null-pointer optimization). The cost on the hot
/// [`Self::send`] path is one register-resident, well-predicted
...
pub(crate) struct WakingSink {
    ...
    /// edge. The drop-then-wake ordering is the structural floor of
    /// the actuator-gone signal — see the struct rustdoc for the
    /// race-window rationale. Steady-state [`Self::send`] reads via
    /// [`Option::as_ref`]; the only call site that takes the inner
    /// value is the [`Drop`] impl below.
...
impl WakingSink {
    ...
    ///
    /// The [`Option::as_ref`] branch surfaces `SendError::Disconnected`
    /// if the sink's [`Drop`] has already run (only reachable from a
    /// hypothetical post-Drop send — every production caller holds
    /// the sink by reference for the call's lifetime, so the branch
    /// is unreachable in steady state and well-predicted regardless).
...
impl Drop for WakingSink {
    ...
    ///
    /// [`Option::take`] returns the inner [`Sender<Input>`] in an
    /// [`Option`]; the returned value is dropped at the end of the
    /// statement (no binding) which drops the sender. The drop
    /// transitions the paired [`crossbeam::channel::Receiver`] into
    /// the disconnected state — the driver's next `try_recv` returns
    /// [`crossbeam::channel::TryRecvError::Disconnected`] rather than
    /// `Empty`.
    ///
    /// The subsequent [`WakeHandle::wake`] pulse is best-effort: a
    /// destroyed Poll (the shutdown path) returns Err which we
    /// discard. The next surviving wake's drain picks up the
    /// disconnect; mio's late-wake-on-destroyed-Poll contract is a
    /// silent no-op so there's nothing to propagate on the way out.
    ///
    /// Ordering matters: if the wake fired BEFORE the sender drop,
    /// the driver could wake, observe `Empty` on the still-connected
    /// channel, sleep again, and strand on the eventual Disconnected
    /// until an unrelated wake fired. Wake-after-close collapses the
    /// race — the wake-and-disconnect pair lands as one atomic
    /// observation from the driver's side.
    fn drop(&mut self) {
        self.tx.take();
        let _ = self.waker.wake();
    }
...
mod tests {
    ...
    /// `mio::Waker` exactly once — the production actuator-gone signal.
    ///
    /// The test isolates the close-then-wake ordering: any queued
    /// message must drain cleanly (`Ok(_)`) BEFORE the receiver
    /// observes `Disconnected`, and the post-drop `Poll::poll` must
    /// return immediately with `TOKEN_WAKER` ready (proving the
    /// Drop-fired wake edge landed).
    #[test]
    fn drop_closes_channel_then_pulses_wake() {
        ...
        // Send one Input so the channel has a queued message; the
        // Drop-fired Disconnected must surface AFTER the drain.
        ...
        .expect("send");
        // Drain the prior send-then-wake pulse so the assertion below
        // observes the Drop-fired edge in isolation.
        ...
        // Drop-fired wake edge: the next poll returns immediately
        // with TOKEN_WAKER ready.
        ...
        // The queued send arrives normally (Ok), then the receiver
        // observes Disconnected on the next try_recv — proving the
        // sender drop ran BEFORE the wake (and therefore happens-before
    ...
    /// Drop on an empty channel still pulses the wake edge AND
    /// surfaces Disconnected immediately on the next `try_recv` —
    /// the pristine actuator-startup-fails scenario.
    #[test]
    fn drop_on_empty_channel_surfaces_disconnected_immediately() {
        ...
        // Wake edge fired on Drop, so the next poll returns
        // immediately with TOKEN_WAKER ready.
        let mut events = Events::with_capacity(4);
        poll.poll(&mut events, Some(Duration::from_secs(2)))
            .expect("Drop-fired wake unblocks poll");
        assert!(
