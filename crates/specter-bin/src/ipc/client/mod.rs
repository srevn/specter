...
//!   `fail_response`). Single source of timeout, the `specter <verb>:`
//!   prefix, the Ok/non-Ok dispatch the unit-ack verbs share, and the
//!   response-tail rendering every data verb reuses.
...
//! `connect::round_trip(verb, request)` → match the response → emit.
//! The data verbs (`status`, `list`, `show`) collapse the human/JSON
//! emission into [`connect::emit_human_or_json`] (`status` / `list`
//! via the exit-0 [`connect::render_response`] wrapper); the unit-ack
//! verbs (`disable`, `enable`, `reload`, `absorb`) collapse the
//! Ok/non-Ok arm into [`connect::one_shot_unit`]. The two streaming
//! handlers reach [`subscribe::Subscription`] instead of `round_trip`
//! because the post-ack horizon is per-event, not per-call.
//!
//! # Visibility
//!
