//! Operator IPC protocol — wire-side request shape, response carriers, wire-id newtype, and the
...
//! ```
//!
//! [`WireRequest`] is the only request type the daemon ever sees: the mio-reactor driver reads
//! bytes off each per-conn stream, parses one `WireRequest` per line, and dispatches inline on the
//! same thread — no channel envelope, no per-request reply channel, no per-conn worker thread.
//!
//! Deserialize-only: operators address by name, not by id, so the daemon refuses to admit `WireId`
//! values from clients.
//!
//! # Response shape
//!
//! [`ResponsePayload`] is internally tagged on `kind`; every variant flattens into a single JSON
//! object keyed by `kind`, symmetric with [`super::wire::WireDiagnostic`]'s `diag` tag. Operators
//! filter the entire wire surface with one `jq` pattern.
//!
//! # Visibility
//!
//! Every export is `pub(crate)`. The driver IPC drain, projection helpers, and client verbs consume
//! each type at its own point of use.
...
/// constructs at write time.
///
/// The type carries name-only addressing by construction: no variant holds a [`WireId`] field, so
/// clients cannot round-trip a slotmap key whose generation has since expired engine-side (a fresh
/// disable/enable would reuse the slot index and the cached id would silently resolve to a
/// different Sub). The structural floor lives in the field shapes, not in a missing `Serialize`
/// impl — both directions of the wire round-trip the same JSON object.
///
/// Tagged internally on `op`; both the tag value and the field names use `snake_case` so the wire
/// form matches the operator vocabulary (`{"op":"status"}`, `{"op":"show","name":"foo"}`).
///
/// Unknown-field strictness lives at the wire boundary, not on the type:
/// [`crate::ipc::framing::parse_strict`] round-trip-validates every incoming request line so a typoed
/// JSON (`{"op":"subscribe","names":"build"}`) reaches the handler as [`WireErrorCode::Malformed`]
/// rather than silently dropping the typo'd field. The boundary discipline applies uniformly to
/// `WireRequest` (daemon read) and [`ResponsePayload`] (client read); `WireDiagnostic` is
/// deliberately exempt — streamed diags stay forward-compatible with older `tail`/`wait` clients.
...
pub(crate) enum WireRequest {
    ...
    /// Arm an `absorb` window on the named watch's Profile — fold the next fireable burst into the
    /// baseline instead of firing (the echo-suppression signal for an expected replication). `name`
    /// resolves to a Profile driver-side; the wire carries no id, same name-only addressing as
    /// every other verb. `duration_ms` is the window length: `None` ⇒ the engine's default (one
    /// `settle` interval, consume-on-first); `Some(ms)` ⇒ a time-boxed window. The wire has no
    /// `Duration` type, so the duration rides as a scalar millisecond count (matching
    /// [`WireTime`]'s hand-rolled-scalar discipline); the driver rebuilds the `Duration` — and
    /// clamps it — before lifting `Input::ArmAbsorb`.
    Absorb {
        name: CompactString,
        #[serde(default)]
    ...
    Reload,
    /// Subscribe to the diagnostic stream. `name` is optional — `None` is an unfiltered tail;
    /// `Some(n)` scopes the subscription to events naming that Sub, collapsing the historical
...
///
/// Internally tagged on `kind`, `snake_case`. Every variant serializes as a flat JSON object —
/// newtype-around-struct variants (`Status`, `List`, `Show`) inline their carrier's fields next to
/// `"kind"`; struct variants (`SubscribeAck`, `Err`) carry their own fields; unit variants (`Ok`)
/// carry only the tag.
///
/// `Deserialize` is symmetric with `Serialize` so client code can parse responses back from the
/// wire — every variant payload carries `Deserialize`. The wire schema is one canonical shape; the
/// same JSON object the daemon writes is what the client reads.
///
/// Unknown-field strictness on the client side lives at the wire boundary via
/// [`crate::ipc::framing::parse_strict`] — a future daemon field this client doesn't know about
/// reaches the verb handler as a parse error rather than silently dropping the value. Symmetric
/// with the daemon-side request gate (see [`WireRequest`]'s rustdoc).
#[derive(Clone, Debug, Deserialize, Serialize)]
...
pub(crate) enum ResponsePayload {
    ...
    List(ListResponse),
    Show(ShowResponse),
    /// Ack for a Subscribe; `sub` carries the resolved [`WireId`] when the request named a Sub,
    /// `None` for the unfiltered tail.
    SubscribeAck {
        sub: Option<WireId>,
    },
    /// Structured error: `code` is a closed-set [`WireErrorCode`] vocabulary the daemon emits —
    /// clients branch on it. `error` is the human-readable amplification.
    Err {
        code: WireErrorCode,
        error: String,
    },
...
///
/// Every variant is a unit value; `#[serde(rename_all = "snake_case")]` makes the wire form a bare
/// quoted token (`"unknown_sub"`, `"already_subscribed"`, …) symmetric with the rest of the
/// wire-projection layer's discipline ([`WireFsEvent`](super::wire),
/// [`WireStateLabel`](super::wire), etc.). Serialize and Deserialize validate the same finite set,
/// so a client receiving a daemon-emitted error parses into the same typed variant the daemon
/// constructed — no host type behind the field, no separate adapter, no `Cow`.
///
/// [`Display`](std::fmt::Display) writes the exact wire form via [`Self::as_str`], so the client
/// renderer (`eprintln!("specter <verb>: {code}: {error}")`) emits the wire token verbatim.
///
/// The vocabulary is intentionally closed (no `#[serde(other)]` fallback): a client that hits a
/// daemon emitting a code it doesn't understand surfaces the failure loudly through the verb's
/// catch-all arm (`unexpected response: ...`), rather than silently parsing a future code into an
/// opaque sink. Request/response carriers favor strictness; diag fan-out favors forgiveness.
...
pub(crate) enum WireErrorCode {
    ...
    DynamicSubNoOp,
    /// `enable` / `disable` precondition: the targeted Sub is not in the state the verb expects
    /// (`enable` against a Sub that is not runtime-disabled, or `disable` against one already
    /// disabled).
    NotDisabled,
    /// `enable` cleared the runtime override but the Sub is also TOML-disabled (or absent from the
    /// file entirely) — the daemon cannot re-attach until the operator edits the config and reloads.
    TomlDisabled,
    /// Connection cap reached — too many concurrent operator clients.
    Busy,
    /// Daemon refused a response whose serialized bytes would exceed the per-conn write-queue
    /// accept cap. Symmetric with [`Self::Busy`]: both are cap-class failures, both close the conn
    /// after the structured Err line flushes, both let operator scripts branch on a single
    /// closed-vocabulary token rather than parsing an `UnexpectedEof`.
    ///
    /// Emitted by [`crate::driver::Hub::enqueue_response`]'s Refused arm; the structured line is
    /// queued into the per-conn reserve (sized so the Err always fits regardless of queue state)
    /// and `close_after_flush` is armed by the upstream `push_response`. The `error` field carries
    /// the byte counts as `response of N bytes exceeds per-conn cap of M bytes`, renderable
    /// verbatim by the operator client.
    ResponseTooBig,
    /// Request line failed JSON parse, or carries an unknown `op`.
    Malformed,
    /// `Subscribe` invoked on a conn that already flipped to subscriber role. Subscribe is one-shot
    /// per conn — a repeat call would silently overwrite the prior `name` filter and drop any
    /// pending back-pressure accounting. The handler refuses with this structured error so the
    /// operator sees a deterministic failure instead of an invisible state mutation.
    ///
    /// The wire vocabulary is pinned by `tests::wire_error_code_round_trips_every_variant`; the
    /// handler-side gate that reaches this variant lives on [`crate::driver::EngineDriver`]'s
    /// Subscribe arm.
    AlreadySubscribed,
    /// Daemon is winding down — mutating verbs (`Reload`, `Disable`, `Enable`, `Absorb`) refused.
    /// The gate fires iff `EngineDriver::first_term.is_some()` — the operator pulsed SIGINT /
    /// SIGTERM and the actuator is in the middle of its SIGTERM → grace → SIGKILL → reap-drain
    /// pipeline. Read-only verbs (`Status`, `List`, `Show`, `Subscribe`) continue working so
    /// operators can observe in-flight shutdown via `specter tail`; the engine still emits
    /// diagnostics during shutdown (late effect completions, post-fire rebase ceilings, etc.).
    ///
    /// Operator-script branch: a `code == "shutting_down"` Err surfaces "daemon is going away;
    /// don't retry against this PID"; the operator can re-issue the verb against the next boot.
    /// Subscribe stays accessible because its mutation is bin-local — flipping `conn.role` doesn't
    /// touch engine state and is safe to enact during shutdown.
    ShuttingDown,
}
...
impl WireErrorCode {
    ...
    ///
    /// Exhaustive `match` — a new variant without a paired arm fails to compile, keeping the wire
    /// vocabulary single-source against `tests::wire_error_code_round_trips_every_variant`'s
    /// JSON-form pin.
    ///
    /// Takes `self` by value: the enum is `Copy` and a single byte wide, so the by-value form is a
    /// register-passing convention rather than the indirection a `&self` borrow would imply.
...
impl std::fmt::Display for WireErrorCode {
    ...
    /// The client's `eprintln!("specter <verb>: {code}: {error}")` reaches here, so the human view
    /// and the JSON form share one source of truth.
...
///
/// The wire form is the bare `u64` returned by [`slotmap::KeyData::as_ffi`] — documented as
/// `(generation << 32) | index`, stable across slotmap minor releases. `Debug` would order
/// differently across versions and is not used here.
///
/// One-way only: every [`From`] impl is *into* `WireId`. There is no `From<u64>` for the four core
/// key types because the daemon must not admit a [`WireId`] from a client: see [`WireRequest`].
///
/// `#[serde(transparent)]` collapses the JSON form to a bare integer (`42`), not a wrapped object
/// (`{"WireId":42}`).
///
/// `Deserialize` rounds out the wire shape so a client-side response carrier (`StatusResponse`,
/// etc.) parses the embedded ids back into `WireId` newtypes. The asymmetry around `From<SubId>`
/// etc. (one-way projections) is preserved: `Deserialize` is the wire shape, the `From` impls are
/// the engine-side projection.
...
///
/// Lives in this module rather than alongside the other wire projections because the discrimination
/// is bin-local: there is no `specter-core` analogue to project from.
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
pub(crate) enum DisabledSource {
    /// Operator ran `specter disable <name>` via IPC and has not yet run `enable`. Cleared by
    /// `enable`, or implicitly by the TOML `[[watch]]` entry leaving the file entirely.
    Runtime,
    /// `[[watch]]` entry carries `enabled = false`. Cleared by the operator editing the config and
    /// triggering a reload.
    Toml,
}
...
pub(crate) struct StatusResponse {
    ...
    /// Wall-clock of daemon boot — sampled in the same constructor as `start_instant` so the two
    /// anchors agree.
    pub(crate) start_wall: WireTime,
    /// Cumulative successful reloads (SIGHUP + auto-reload + IPC).
    pub(crate) reload_count: u64,
    /// Most-recent reload — wall-clock + attribution as one observable. `None` before the first one
    /// fires. Flattens on the wire so the JSON form carries `last_reload_at` and `last_reload_via`
    /// directly alongside the other fields on the `Some` side; both keys are omitted entirely when
    /// `None`. Mirrors the source-side `Option<crate::driver::state::LastReload>`.
    ...
    /// `config.disabled_names().0.len()` — TOML-disabled rows (`enabled = false`).
...
/// Most-recent successful reload as one wire observable — wall-clock at the reload-record call site
/// plus the attribution enum that drove it. Mirrors `crate::driver::state::LastReload` on the wire;
/// the JSON form stays flat (the two inner fields inline directly alongside [`StatusResponse`]'s
/// siblings via `#[serde(flatten)]`) so operator scripts that parsed `status.last_reload_at` /
/// `status.last_reload_via` continue to read the same byte shape on the `Some` side. The `None`
/// side omits both keys entirely — the type-driven encoding of "never reloaded yet."
///
/// Field renames keep the Rust ergonomics (`lr.at`, `lr.via`) without touching the JSON contract —
/// see [`Self::at`] and [`Self::via`].
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub(crate) struct WireLastReload {
    /// Wall-clock of the most recent successful reload. Serializes as `last_reload_at` on the wire
    /// so the JSON form lines up with the flattened [`StatusResponse`] keys on the `Some` side.
    #[serde(rename = "last_reload_at")]
    pub(crate) at: WireTime,
    /// Trigger of the most recent successful reload. Serializes as `last_reload_via` on the wire so
    /// the JSON form lines up with the flattened [`StatusResponse`] keys on the `Some` side. Typed
    /// enum (not `&'static str`) so a future variant on [`super::wire::WireReloadTrigger`] is a
    /// compile error at the declaration site, in the same shape as the rest of the wire projection
    /// layer.
    #[serde(rename = "last_reload_via")]
    pub(crate) via: WireReloadTrigger,
}

/// `specter list` response — every operator-declared Sub keyed by name. Attached, runtime-disabled,
/// and TOML-disabled populations union into one alphabetically-ordered row set.
#[derive(Clone, Debug, Deserialize, Serialize)]
...
/// (Active / Disabled / Unknown) carry their own field set without an outer envelope.
///
/// The outer envelope is [`ResponsePayload::Show`], which tags on `kind`; the inner `status` tag
/// flattens alongside. Both tags appear in the same JSON object (`{"kind":"show","status":"active",
/// "name":"foo",…}`) — they do not collide, and the operator can dispatch on either independently.
#[derive(Clone, Debug, Deserialize, Serialize)]
...
pub(crate) enum ShowResponse {
    ...
    /// Sub is not operator-declared at all (no `[[watch]]`, no runtime disable record).
    Unknown { name: String },
}

/// [`WireRequest`] is structurally infallible to serialize: every variant carries only plain-data
/// fields ([`CompactString`] / unit), every derived `Serialize` walks through `serialize_str` /
/// `serialize_unit` against in-memory values, and no field uses a `serialize_with` adapter that
/// could fail. Marks the client-side request path ([`crate::ipc::client::connect::write_request`])
/// safe for the `Vec<u8>`-returning wrapper without an `.expect`-at-a-distance.
impl InfallibleSerialize for WireRequest {}

/// [`ResponsePayload`] is structurally infallible to serialize: the variant payloads are plain-data
...
pub(crate) struct SubDetails {
    ...
    /// Operator-display state (`StateLabel`). `None` signals the same engine-invariant breach
    /// [`ListRow::state`] does: the attached Sub's Profile is gone. Symmetric across `show -o json`
    /// and `list -o json` — operators decode the breach identically on both verbs, and the IPC
    /// layer projects the missing Profile as `None` rather than panicking the daemon out from under
    /// every concurrent operator session. The breach loudness lives engine-side (the `ProbeSlot`
    /// tripwire, the registries' `debug_assert!`), not in the projection layer.
    pub(crate) state: Option<WireStateLabel>,
    /// Anchor path (`engine.tree().path_of(profile.resource)`). `None` signals "anchor vanished /
...
mod tests {
    ...
    /// Every slotmap key family projects through `KeyData::as_ffi()` to the same canonical
    /// `WireId(u64)`. A regression in any `From` impl, or a future-added key family that grew an
    /// impl returning a different shape, fails here.
    ///
    /// The canonical value comes from `KeyData::as_ffi()` itself, not from the raw bits handed to
    /// `from_ffi`: `KeyData::from_ffi` normalizes the generation's high bit so a freshly minted key
    /// is non-default, so an arbitrary `u64 → from_ffi → as_ffi` round-trip is not guaranteed to
    /// preserve every bit.
    #[test]
    ...
    /// `WireRequest` deserializes the unit-shaped verbs, the name-bearing struct variants, and the
    /// absent-name form of Subscribe. `#[serde(default)]` on `Subscribe.name` is the load-bearing
    /// detail — missing field yields `None`, not parse failure.
    #[test]
    ...
    /// `WireRequest::Absorb` round-trips through serde with both the absent and present
    /// `duration_ms`. `#[serde(default)]` on `duration_ms` is the load-bearing detail — a bare
    /// `{"op":"absorb","name":"foo"}` parses with `duration_ms: None` (the engine's consume-on-first
    /// default), and an explicit `Some(n)` survives the serialize→deserialize cycle.
    #[test]
    ...
    /// A typoed `op` is a parse failure — the wire surface refuses to silently match nothing.
    /// Catches operator typos (e.g. `sub_fire` for `status`) at the daemon boundary.
    #[test]
    ...
    /// `parse_strict` rejects a typoed payload field on a struct variant — the bug class the
    /// boundary gate exists to prevent. `{"op":"subscribe","names":"build"}` (note the trailing
    /// `s`) would silently parse with `name = None` under bare `serde_json::from_slice` and the
    /// daemon would subscribe unfiltered, ignoring the operator's intent. With the boundary gate
    /// the typo surfaces as an unknown-field rejection the [`crate::driver::ipc`] handler turns
    /// into [`WireErrorCode::Malformed`].
    ///
    /// Bare `from_slice` is exercised first to pin the bug shape (silent accept) the boundary gate
    /// fixes; the strict parse is the structural floor.
    #[test]
    ...
    /// `parse_strict` rejects an unknown top-level field on a [`ResponsePayload`] — the daemon-bug
    /// class the client-side boundary gate ([`crate::ipc::client::connect::read_response`])
    /// surfaces as [`std::io::ErrorKind::InvalidData`] rather than silently dropping the value.
    /// Symmetric protection with the daemon-side request gate.
    #[test]
    ...
    /// `ResponsePayload`'s internal tag is the load-bearing wire commitment: every variant flattens
    /// into one JSON object keyed by `kind`. A retrofit to external tagging would change the
    /// operator-visible shape and fail this test.
    ///
    /// The Err arm exercises one [`WireErrorCode`] variant inline so the test's reach across
    /// [`ResponsePayload`] stays intact; every-variant coverage of the error-code vocabulary lives
    /// in [`wire_error_code_round_trips_every_variant`].
    #[test]
    ...
    /// Every [`WireErrorCode`] variant projects to its snake_case wire token and round-trips
    /// identically through serde + [`WireErrorCode::as_str`] + [`std::fmt::Display`].
    ///
    ...
    fn wire_error_code_round_trips_every_variant() {
        ...
        // Spot-check the embedded shape: a `code` field deserialized from a daemon-emitted Err line
        // yields the matching variant.
    ...
    /// [`DisabledSource`] follows the same `as_str` ↔ serde-rename ↔ `Display` discipline as
    /// [`WireErrorCode`] — every variant projects to the wire token and round-trips identically.
    ...
    /// `StatusResponse.last_reload` flattens onto the wire — `Some` inlines `last_reload_at` and
    /// `last_reload_via` next to the peer fields; `None` omits both keys entirely. No wrapper key
    /// (`last_reload`) ever appears in the JSON form, on either side. The wire shape is the lift's
    /// load-bearing contract: operator scripts that read `status.last_reload_at` /
    /// `status.last_reload_via` keep working unchanged on the `Some` side.
    #[test]
    ...
    /// `StatusResponse` round-trips through serde with the flattened `last_reload` pair preserved
    /// on the `Some` side. The serialize-deserialize chain reaches the wire field renames (`at` ↔
    /// `last_reload_at`, `via` ↔ `last_reload_via`) in lockstep — a rename drift on either side
    /// fails this test rather than silently dropping the value.
    #[test]
    ...
    /// A partial wire shape — one of the two `last_reload_*` keys present, the other absent — fails
    /// the strict boundary parse. The structural witness operates in two layers:
    ///
    /// 1. Bare [`serde_json::from_slice`] over the flattened `Option<WireLastReload>` silently
    ///    collapses partial input to `None`; serde's `flatten + default` machinery does not surface
    ///    the missing-sibling as a parse error.
    /// 2. [`parse_strict`] — the boundary every client response read routes through
    ///    ([`crate::ipc::client::connect::read_response`]) — catches the partial via its round-trip
    ///    walk: the parsed `StatusResponse` round-trips with the orphan key omitted, so the
    ///    original's `last_reload_at` (or `last_reload_via`) is reported as `unknown field` at the
    ///    boundary.
    ///
    /// The wire-side invariant therefore holds at the operator-facing surface even though the inner
    /// deserialize tolerates partial input — pinning both behaviors keeps the test honest about
    ...
    fn status_response_partial_last_reload_shape_caught_at_boundary() {
        ...
        // Layer 1 — bare serde tolerates the partial shape by collapsing to `None`. The behavior is
        // a serde-flatten property, not a load-bearing wire invariant.
        ...
        // Layer 2 — `parse_strict` catches it. The boundary gate every client response read routes
        // through reports the orphan key as `unknown field` because the parsed `StatusResponse`
    ...
    /// partial-shape tests focus on `last_reload`. Defaults are minimal — the surrounding fields
    ...
    /// Serialize a fully-populated [`StatusResponse`] and drop `drop_key` from the JSON object,
    /// yielding the partial wire shape the strict boundary test asserts against. Building the bytes
    /// through serde rather than hand-typing the JSON keeps the test resilient to a future field
    /// rename / reorder on `StatusResponse`.
