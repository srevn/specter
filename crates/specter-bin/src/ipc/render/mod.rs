...
//! is allocation-free.
//!
//! Renderers paint semantic tokens via a [`style::Styler`] threaded as
//! their trailing argument: `Styler::Active` brackets tokens with ANSI,
//! `Styler::Plain` is a byte-identical passthrough. The Styler is
//! resolved once per output stream by [`style::resolve`]; `-o json`
//! output is never styled.
//!
...
//! - [`style`] — the semantic color vocabulary (palette, `Styler`,
//!   gating) shared across all four.
//!
//! # Visibility
//!
...
/// A left-aligned label cell painted with [`style::LABEL`] — the
/// shared key-column primitive for `status` and `show`, where a label
/// is padded to a fixed width and the value follows on the same line.
/// The padding is applied to the plain text ([`PadRight`]) and the
/// paint wraps the padded result, so column alignment survives styling.
#[must_use]
