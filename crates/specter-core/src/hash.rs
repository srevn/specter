//! Deterministic 64-bit and 128-bit fingerprint primitives, built on
//! SipHash-2-4 with the algorithm's default (unkeyed) initialisation.
//!
//! This is the *only* hashing in `core`/`engine` (I7). All call sites —
//! `config_hash`, `Snapshot::content_hash`, `dir_hash`, `leaf_hash`,
...
//! adversarial-collision guarantees: every hash in Specter is computed
//! over operator-controlled inputs (config files, kernel-observed
//! filesystem state) and consumed in-process for equivalence checks and
//! `BTreeMap` lookups. SipHash-2-4 is chosen for its prefix-freeness,
//! well-studied diffusion, and small state — properties the hierarchical
//! `dir_hash` fold leans on — not for keyed authentication. Output is
//! pinned by golden tests against algorithm drift (e.g. a `siphasher`
//! upgrade) and against `Hash` impl semantics drift in `std`.
//!
...
/// Fresh 64-bit SipHash-2-4 fingerprint hasher.
#[must_use]
pub fn hasher() -> Sip64 {
    Sip64::new()
}

/// Single-shot 64-bit fingerprint of any `Hash` value.
#[must_use]
...
/// Fresh 128-bit SipHash-2-4 fingerprint hasher.
///
...
/// Single-shot 128-bit fingerprint of any `Hash` value.
#[must_use]
...
/// Pinned via the `pinned_hash_128_for_foo` golden — drift in the encoding
/// changes every computed 128-bit fingerprint and must be accompanied by
/// a golden refresh.
...
mod tests {
    ...
    /// Golden test: pins the SipHash-2-4 fingerprint output for the input
    /// `"foo"` against the stable `Hash for str` implementation. If this
    /// assertion fires, either the `siphasher` algorithm changed (crate
    /// upgrade) or `Hash for str` changed semantics (Rust ABI break);
    /// either branch requires a deliberate golden refresh.
    ///
    ...
    #[test]
    fn pinned_hash_for_foo() {
        let h = hash_one("foo");
        assert_eq!(
            h, 0xe1b1_9adf_b2e3_48a2,
            "hash_one(&\"foo\") drifted; siphasher upgrade or Hash impl change. got={h:#018x}",
        );
    }
    ...
    /// Mirrors `pinned_hash_for_foo` for the 128-bit hasher: pins the
    /// SipHash128-2-4 fingerprint output for the input `"foo"`. Drift
    /// changes every cached `dir_hash`/`leaf_hash` this binary computes.
    #[test]
    fn pinned_hash_128_for_foo() {
        let h = hash_one_128("foo");
        assert_eq!(
            h, 0x0bcf_4e56_fefe_511b_8208_f531_8ff6_2bbc,
            "hash_one_128(&\"foo\") drifted; siphasher upgrade, encoding change, or \
             Hash impl change. got={h:#034x}",
        );
    }
